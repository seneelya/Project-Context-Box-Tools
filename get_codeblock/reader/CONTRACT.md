...
- **survey** (ни `--outline`, ни `--query`) — «ты грепнул, вот доказательство, вот куда это ведёт».
  На каждую группу хитов с одинаковой лестницей: сначала лестница OUTERMOST→INNERMOST (lvl 1 первым,
  читается сверху вниз как код), затем — ПОСЛЕ неё — точная строка(и) из файла для каждого хита
  этой группы (raw, `.strip()`, без обогащения — буквальный grep-хит, не карта), вложенная на один
  уровень глубже самого внутреннего рунга («хит лежит внутри своего блока» — визуально ниже и правее
  него, не наоборот). Соседние хиты, у которых лестница ПОБИТОВО совпадает (те же `level/start/end`
  на каждой ступени), делят её ОДНУ печать (не глобальный дедуп по всему батчу — только по соседним
  прогонам; хиты того же блока не подряд по списку получат лестницу повторно — сознательный
  компромисс простоты, см. `_run_survey_batch`).
...
**Версия.** `core.VERSION` (сейчас `0.4.0`) — с реальными внешними потребителями (Hermes-агенты)
