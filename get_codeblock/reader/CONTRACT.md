...
7. **Выданный блок ОБЯЗАН содержать строку.** Любой рунг `get_blocks(path, line)` удовлетворяет
   `start ≤ line ≤ end`. «Ближайший» блок, не покрывающий строку, — запрещён (был баг отступной
   эвристики на строке внутри top-level списка). Не нашли настоящий контейнер — честный module-scope
   `[1, N]`, а не выдуманный сосед. Отступной путь фильтрует рунги этим инвариантом в `get_blocks`.
