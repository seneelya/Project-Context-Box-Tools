...
- **survey** (ни `--outline`, ни `--query`) — «ты грепнул, вот доказательство, вот куда это ведёт».
  ОДНА объединённая карта на весь батч, тем же приёмом, что и outline batch: рунги ВСЕХ лестниц
  сливаются по `(start, end)` (не только у соседних по списку хитов — у любых, даже далёких друг
  от друга), сортируются (start↑, end↓ — outer раньше inner) и печатаются один раз, читается
  сверху вниз как код. Точная строка из файла для каждого хита (raw, `.strip()`, без обогащения —
  буквальный grep-хит, не карта) вставляется СРАЗУ ПОСЛЕ рунга, в который он попал (`hits_by_block`
  в `_run_survey_batch`), на один уровень глубже — «хит лежит внутри своего блока», не наоборот.
  Несколько хитов в одном и том же рунге идут друг за другом. Общие предки (тот же класс/файл)
  печатаются РОВНО ОДИН раз, даже если хиты лежат в разных, далеко разошедшихся ветках дерева.
...
- **query batch** (`--query --line …`) — резолвим каждый хит, сортируем по позиции в файле
  (ВСЕГДА, не по входному порядку `--line` — порядок хитов после резолва уже не несёт информации,
  позиция — несёт), затем СКЛЕИВАЕМ (merge) любые два диапазона, которые касаются или
  перекрываются — **включая нулевой зазор** (`end` одного `+1 == start` другого), не только строгую
  вложенность. Мы возвращаем ФАЙЛ: если диапазон A кончается на строке 46, а B начинается на
  строке 47 — в исходнике между ними нет реального разрыва, значит печатать их как два отдельно
  обрамлённых блока значило бы вписать `Block end:`/`Block level:` ровно там, где в файле шва нет —
  и если этот вывод когда-нибудь склеят обратно в код, это активно неверный шов. Только РЕАЛЬНЫЙ
  зазор (хотя бы одна непокрытая строка между диапазонами) оставляет два отдельных блока. Вложенность
  (один диапазон целиком внутри другого) — тот же merge, просто без расширения границ. Эскалация
  уровнями — обычное дело, когда несколько хитов упираются в ОДНОГО и того же предка, или один хит
  эскалирует в предка, который уже накрывает диапазон другого — без merge это означало печать одного
  и того же (или перекрывающегося) куска файла НЕСКОЛЬКО раз подряд. Не косметика: 5 хитов,
  эскалировавших в 6571-строчную функцию, дали бы 30000+ строк одного и того же текста из одного
  вызова.

  **Формат заголовка блока (финальный, после нескольких заходов) — две РАЗНЫЕ вещи, никогда не
  смешанные в одном числе:**
  ```
  #■BLOCK : A-B
  <тело, ровно A-B>
  #■END : B
  ```
  или, если в этот диапазон склеилось больше одного исходного резолвнутого блока — ОДНА (и только
  одна — вспомогательная инфа никогда не растягивается на несколько строк, чтобы было "удалил и
  забыл") добавочная часть на той же строке BLOCK:
  ```
  #■BLOCK : 35-51  = ranges :  Level 2  35-51,  Level 5  41-49
  ```
  `■BLOCK : A-B` / `■END : B` — обрамляющая нумерация, ВСЕГДА правда: это ровно тот диапазон, который
  реально нарезан и напечатан ниже, никогда не заявление о глубине. `■` — маркер "эту строку написал
  инструмент, не файл" (никогда не встречается в начале реальной строки кода — та же логика, что и
  комментирование зелёных TTY-подсказок, только явный однозначный символ вместо факта комментария).
  `= ranges : ...` — метаданные о том, из чего эта печать собрана (`Level {level}  {start}-{end}`,
  через запятую); печатается ТОЛЬКО когда была склейка (>1 исходного блока) — если склейки не было,
  этого хвоста просто нет, нечего добавлять сверх самого BLOCK. Раньше пробовали одним числом уровня
  описать весь склеенный спан (сначала минимальным/внешним среди вошедшего) — тоже ложь, просто более
  скромная: спан реально бывает на разных глубинах внутри одного напечатанного тела. Также убрали
  `[i/n]`-счётчик — он ломал фиксированную грамматику строки (то есть, то нет), а пользы давал мало
  (число блоков и так видно по количеству строк `BLOCK`).

  Рассматривали как раз этот алгоритм («зеркало строк файла в памяти» — булев массив покрытия по
  каждой строке, потом сплошные закрашенные диапазоны как единицы вывода) и сначала ОТКЛОНИЛИ его в
  пользу «сортировка + drop-if-contained» (без расширения на касание) — опасаясь, что два РАЗНЫХ
  sibling-блока без зазора между собой (два разных объекта, конец одного = начало+1 другого) сольются
  в один блок с одним заголовком, потеряв границу между ними. Живое ревью поправило: это НЕ баг, это
  и есть корректное поведение — мы возвращаем файл, а не реестр объектов; фейковый шов там, где в
  файле шва нет, хуже потери одного заголовка. `Sort + merge-on-touch` — финальный алгоритм.

  Тело печатается РОВНО ОДИН раз на каждый склеенный диапазон. Нумерация — `[i/n]`, но `n` = число
  диапазонов ПОСЛЕ склейки, не число хитов — так дедуп никогда не путает счётчик. Никаких hit-строк
  и hit-счётчиков в заголовке (query не про «докажи что попал», это уже про «дай тело») и никакого
  текстового label рядом с `Block level: K range: A-B` — он бы всего лишь повторил первую строку тела
  одной строкой ниже, лишний риск разойтись. Плохой хит не роняет батч — `#ERROR: …` (без «hit»,
  просто номер строки) и идёт дальше; ненулевой exit-код, если хоть один был.

**Одиночный `--line N` — НЕ отдельный режим.** Он тоже идёт через `_run_survey_batch`/
`_run_query_batch` (массив из одного элемента) — раньше был отдельный «скалярный» рендер
(`You hit in block lvl K:` / `[N]` без File-заголовка), теперь удалён целиком, не закомментирован:
два похожих-но-разных пути для «одного и того же» читаются как баг внешним потребителем скорее,
чем как фича. Заголовок корректно склоняет: `1 hit` / `N hits` (`_hits()`).

**Стрелка `→` всегда прижата к `|`, независимо от своего `indent`.** Она означает «вот хит», не
глубину — стайрекейс-позиция уместна только у маркера РЕАЛЬНОГО уровня. Раньше стрелка тоже
стайрекейсилась вместе с блоками, и в одном выводе с разноглубокими группами съезжала от `|`
(например строка ошибки с `indent=0` рядом с хитами `indent=1`) — вводило в заблуждение, будто
стрелка тоже показывает глубину. Текст рядом со стрелкой по-прежнему получает `indent`-отступ —
глубина видна через label, не через положение маркера.
...
обе колонки в `|`-рамке; и колонка-маркер, и label получают ЛЕСЕНКУ вложенности — не по реальной
глубине файла (может быть 10, а показываем 3 строки — раздувать отступ под неё смысла нет), а по
позиции СТРОКИ В ЭТОМ конкретном выводе (`indent` = 0 у самого внешнего показанного рунга).
Ширина/лесенка считается ОДИН раз на весь вывод (не на каждый хит/группу отдельно — старая версия
считала `mw` локально на лестницу, отсюда разъезд колонок между хитами разной глубины; это первый
баг, который поймал живой прогон). Маркер-колонка — «лестница»: чем глубже строка, тем ближе
её маркер к `|` (глубокий рунг сдвинут вправо на `indent` пробелов); label получает синтетический
отступ `'  '*indent` — читается почти как исходный код с вложенностью, без доверия к реальным
пробелам файла (ненадёжны — табы/1-пробел/что угодно). Строка-хит и строка ошибки используют один
символ-маркер `→` в колонке уровня вместо номера (никогда не спутать со `Block level`), и в
колонке диапазона — тот же `→` перед номером строки (`→ N`, не `A-B`): одно число за стрелкой
читается как "точно эта строка", не спутать с диапазоном. Раньше там был индекс хита (`i> LINE`) —
убрали: индекс не давал полезной информации (позиция в `--line` уже не нужна после резолва), а
`>` визуально читался как ложное числовое сравнение ("1 > 41", что вообще-то неправда):
```
 1  |   9-727| public class Settings : AutoConfiguration  /// <summary>...
  2 |  74-100|   public class ServerMaintenanceData : AutoConfiguration  /// <summary>...
   →|    → 75|     public class ServerMaintenanceData : AutoConfiguration
```
Зелёная легенда (`emit_legend`, TTY-only — «'Block level: K' = real depth ... --query = its
text») печатается и в survey/query batch, как и в скалярном режиме — потерялась в первом заходе
батча (не удалялась осознанно, просто забыли протащить вызов), восстановлена. Outline batch её не
печатает — как и одиночный `--outline`, у него свой TTY-хинт.

Исключение из «hit на один indent глубже своего innermost-рунга» — общее правило, не два частных
случая: сравниваем `handler.line_level(ln)` (реальный логический уровень ЭТОЙ строки) с уровнем
самого innermost-рунга. Если `line_level(ln) <= innermost['level']` — хит НЕ внутри тела рунга,
он получает ТОТ ЖЕ indent, не +1. Покрывает два разных на первый взгляд случая одним механизмом:
(a) пинпоинт-филлер (`~return_statement`, `start==end==line`) — сама строка и есть весь рунг;
(b) хит попал на СОБСТВЕННЫЙ заголовок/преамбулу рунга (`async def foo():` сам, или приклеенный
комментарий над ним, а не что-то внутри тела) — живая находка: `test/Edge/Edge.py:38` (сама строка
`async def ws_reader():`) и `:54` (приклеенная комментарий-преамбула перед `ws_writer`) обе
получали +1, хотя визуально «живут на уровне заголовка», не внутри функции. `get_blocks` в обоих
случаях не добавляет более глубокий рунг именно потому, что строка не в теле — `line_level` это
и обнаруживает без привязки к конкретному языку/бэкенду.
...
Самоидентификация файла — ОДНА функция, `_file_header(file_path, lines)`, всегда `File: path
(N lines)` — все три batch-режима вызывают её, а не строят свою версию строки. Число строк файла
не косметика — по нему сразу видно, во что реально выльется эскалация `--level`, до самого вызова.
Правится в одном месте.
Это касается и человеческих TTY-подсказок (зелёная легенда `emit_legend`, outline-хинты) — они
тоже идут через `c()`: не потому что программный потребитель их видит (не видит, `is_tty`-gate),
а потому что человек может скопипастить кусок терминала прямо в код — некомментированная строка
там сломает синтаксис.

**Неизвестный CLI-токен — жёсткая ошибка, не тихий skip.** До этого непризнанный аргумент молча
пропускался — с массивами это стало реальной дырой: PowerShell разворачивает неэкранированный
`a,b, c` (запятая ЗА КОТОРОЙ пробел) в отдельные argv (`a`, `b`, `c`) вместо одной строки; `--line`
видел только свой непосредственный следующий токен как массив из одного элемента и тихо скатывался
в старый скалярный режим, а остальные числа тихо терялись как «unknown argument». Теперь любой
нераспознанный токен — `Error: unrecognized argument` с подсказкой про кавычки, если похож на
случайно разъехавшийся числовой список.
...
**Версия.** `core.VERSION` (сейчас `0.5.0`) — с реальными внешними потребителями (Hermes-агенты)
любое изменение, влияющее на ВЫВОД (колонки, состав label, семантика флагов), готовит инкремент.
Инкремент — только МЕЖДУ сессиями (в конце текущей, перед компактом), не на каждый коммит внутри
одной сессии — иначе версия скачет быстрее, чем реально стабилизируется формат.

**Осознанно НЕ делаем** (см. REQ-001 §8 для полного разбора): `--head` (избыточен — `--outline`
уже даёт заголовки объектов); диапазоны `--line a-b` (рассмотрели, убрали из v1 — краевые случаи
вроде двух несвязанных top-level объектов делают это отдельной мини-фичой).

`--force` реализован — см. `Vision05__get_codeblock.md` (2026-09-13): `--query` на результате
ниже FLOOR-порога эскалирует параметры вызова (горизонтальные соседи на общем уровне, дефолты
`ESCALATE_FLOOR/TARGET/CEILING/K` в `CONFIG__TOOLS.py`), `--force` отключает эскалацию, отдаёт
буквально запрошенный диапазон. Капы `--outline` сверху вниз (Vision04, «стена») — по-прежнему
не реализованы, механики нет, отдельный заход.
