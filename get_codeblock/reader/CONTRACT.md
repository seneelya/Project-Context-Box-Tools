...
- **query batch** (`--query --line …`) — резолвим каждый хит, затем сортируем по позиции в файле
  (ВСЕГДА, не по входному порядку `--line` — позиция важна, порядок хитов после резолва уже не несёт
  информации) и выкидываем блок, если он ЦЕЛИКОМ лежит внутри уже сохранённого (его `end` не
  превышает максимальный `end` из уже сохранённых). Эскалация уровнями — обычное дело, когда
  несколько хитов упираются в ОДНОГО и того же предка, или один хит эскалирует в предка, который
  уже накрывает диапазон другого — раньше это означало печать одного и того же (или перекрывающегося)
  куска файла НЕСКОЛЬКО раз подряд, включая частичные вложенные перекрытия, не только точные
  дубликаты диапазона. Не косметика: 5 хитов, эскалировавших в 6571-строчную функцию, дали бы
  30000+ строк одного и того же текста из одного вызова.

  Рассматривали (и отклонили) вариант «зеркало строк файла в памяти» (булев массив покрытия по
  каждой строке, потом сплошные закрашенные диапазоны как единицы вывода) — проще на первый взгляд,
  но булev-массив не хранит ЧЕЙ это блок, только факт покрытия строки: два ДЕЙСТВИТЕЛЬНО разных
  соседних блока без зазора между ними (два sibling-объекта, конец одного = начало+1 другого)
  слились бы в один печатаемый блок с одним заголовком, потеряв границу между ними. Sort+drop-if-
  contained даёт тот же эффект схлопывания вложенности, но не трогает по-настоящему разные соседние
  блоки — они оба остаются с собственным заголовком.

  Тело печатается РОВНО ОДИН раз на сохранённый (не выкинутый) блок. Нумерация — `[i/n]`, но `n` =
  число блоков ПОСЛЕ схлопывания вложенности, не число хитов — так дедуп никогда не путает счётчик.
  Никаких hit-строк и hit-счётчиков в заголовке (query не про «докажи что попал», это уже про «дай
  тело») и никакого текстового label рядом с `Block level: K range: A-B` — он бы всего лишь повторил
  первую строку тела одной строкой ниже, лишний риск разойтись. Плохой хит не роняет батч —
  `#ERROR: …` (без «hit», просто номер строки) и идёт дальше; ненулевой exit-код, если хоть один был.
