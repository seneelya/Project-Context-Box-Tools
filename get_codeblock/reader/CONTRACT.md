...
- **query batch** (`--query --line …`) — резолвим каждый хит, сортируем по позиции в файле
  ...
  **Формат заголовка блока — две РАЗНЫЕ вещи, никогда не смешанные в одном числе:**
  ```
  #[i/n] BLOCK : A-B ; members L2 A1-B1  L5 A2-B2
  <тело, ровно A-B>
  #END : B
  ```
  `BLOCK : A-B` / `END : B` — обрамляющая нумерация, ВСЕГДА правда: это ровно тот диапазон, который
  реально нарезан и напечатан ниже, никогда не заявление о глубине. `members` — отдельно, метаданные
  о том, из чего эта печать собрана: один элемент `L{level} {start}-{end}`, если склейки не было
  (тогда `A-B` у BLOCK и у единственного члена совпадают), несколько — если было. Раньше пробовали
  одним числом уровня описать весь склеенный спан (сначала минимальным/внешним среди вошедшего) —
  тоже ложь, просто более скромная: спан реально бывает на разных глубинах внутри одного напечатанного
  тела. Разделение чинит это: обрамление никогда не врёт, а `members` не претендует быть обрамлением.
