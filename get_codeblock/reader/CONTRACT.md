...
9. **Filler-полоса — легитимный контейнер, НЕ ТОЛЬКО на уровне файла.** До этого пункта инвариант #7
   ...
   единственный настоящий контейнер для строк напрямую в ней. Внутри landmark (def/class) это НЕ
   нужно: сам landmark уже адресуем и уже содержит строку — до filler-фолбека дело не доходит.
