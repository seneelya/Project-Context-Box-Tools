...
- **query batch** (`--query --line …`) — резолвим каждый хит, сортируем по позиции в файле
  ...
  вызова. Склеенный диапазон репортит `Block level` как МИНИМАЛЬНЫЙ (самый внешний) уровень среди
  того, что в него схлопнулось — единственная метка, которая остаётся верной для всего диапазона,
  когда он собран из >1 исходного блока.
