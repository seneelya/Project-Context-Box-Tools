...
- **query batch** (`--query --line …`) — резолвим каждый хит, сортируем по позиции в файле
  ...
  вызова. Склеенный диапазон НЕ репортит одно число уровня на весь спан (было — минимальный/самый
  внешний уровень среди схлопнувшегося; поправлено: это тоже ложь, просто более скромная — участок
  реально может быть на разных глубинах внутри одного and того же напечатанного тела). Вместо
  этого заголовок перечисляет ВСЕ настоящие исходные блоки, каждый со своим честным
  `Block level: K range: A-B` — если склейка была из одного блока, строка одна, как раньше; если из
  нескольких — несколько строк подряд под одним `[i/n]`, тело печатается один раз под всеми ними.
  Каждая строка утверждает только то, что верно именно про неё.
