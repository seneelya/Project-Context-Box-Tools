...
- **survey** (ни `--outline`, ни `--query`) — «ты грепнул, вот доказательство, вот куда это ведёт».
  На КАЖДЫЙ хит — своя строка с точной строкой из файла (raw, без обогащения — это буквальный
  grep-хит, не карта), затем лестница объемлющих блоков вверх до lvl 1. Соседние хиты, у которых
  лестница ПОБИТОВО совпадает (те же `level/start/end` на каждой ступени), делят её ОДНУ печать —
  их строки-хиты идут подряд, лестница печатается один раз после них (не глобальный дедуп по
  всему батчу — только по соседним прогонам; хиты того же блока не подряд по списку получат
  лестницу повторно — сознательный компромисс простоты, см. `_run_survey_batch`).
...
- **query batch** (`--query --line …`) — тело блока (по broadcast-адресу) на каждый хит,
  ...
  ненулевой exit-код, если `Y > 0`. Строка самого хита здесь НЕ печатается отдельно — тело блока
  уже её содержит.

**Формат строк лестницы/дерева — ОДИН рендерер на survey и outline batch** (`_render_boxed_rows`,
«тупо одинаково», не два похожих-но-разных пути): колонка уровня, колонка диапазона, текст —
обе колонки в `|`-рамке, ширина считается ОДИН раз на весь вывод (не на каждый хит отдельно —
старая версия так и делала, отсюда разъезд колонок между хитами разной глубины). Строка-хит и
строка ошибки используют один символ-маркер `→` в колонке уровня вместо номера (никогда не
спутать со `Block level`), и `i> LINE` в колонке диапазона вместо `start-end`:
```
→|   1> 75|     public class ServerMaintenanceData : AutoConfiguration
→|   2> 76|     {
2|  74-100| public class ServerMaintenanceData : AutoConfiguration  /// <summary>...
1|   9-727| public class Settings : AutoConfiguration  /// <summary>...
```
**Текст label для лестницы = ровно тот же текст, что даёт `--outline`** (с приклеенным
комментарием/докстрингом, если есть), не сырой `label` из `get_blocks` (который — просто первые
80 символов строки заголовка). Источник — `_outline_label_index()`: ОДИН вызов `handler.outline()`
на весь файл, индексированный по `(start, end)`; ladder-рунги ищут в нём свой текст, при
отсутствии — падают на сырой `label`. Дороже одного вызова `get_blocks` за хит, но раз навсегда:
два похожих, но разных пути формирования подписи путают внешних потребителей (замечено на живых
ревью — воспринимается как баг, не как фича), поэтому единственность важнее цены. Оптимизация
(не пересчитывать `outline()` за вызов, если сможем закешировать) — отдельный заход, не блокирует.
...
REQ-001/этот файл фиксируют состояние решения СЕГОДНЯ, не окончательную истину — рендер выше уже
второй заход (первый — текстовая строка `same ladder as [j/n] (×K hits)` — заменён этим боксовым
форматом после живого ревью реального вывода). Меняется по мере использования, без драмы.

**Версия.** `core.VERSION` (сейчас `0.2.0`) — с реальными внешними потребителями (Hermes-агенты)
любое изменение, влияющее на ВЫВОД (колонки, состав label, семантика флагов), инкрементит её.
Не трогать без причины — не переименование ради переименования.
