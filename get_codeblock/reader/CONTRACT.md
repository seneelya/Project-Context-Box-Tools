...
Исключение из «hit на один indent глубже своего innermost-рунга»: если innermost-рунг УЖЕ
пинпоинт этой же строки (`start == end == line`, например `~return_statement` filler-полоса,
которая и есть ровно эта строка) — хит получает ТОТ ЖЕ indent, что и этот рунг, не +1: они
описывают одну и ту же строку, не контейнер-и-содержимое, лесенка между ними была бы неверной.
