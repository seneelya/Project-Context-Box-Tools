...
- **query batch** (`--query --line …`) — та же merge-по-`(start,end)` техника, что и у
  survey/outline, только дедуп идёт по УЖЕ РЕЗОЛВНУТОМУ блоку (после `resolve()`, не по сырому
  хиту): эскалация уровнями — обычное дело, когда несколько хитов упираются в ОДНОГО и того же
  предка (например все — в общий родительский класс/функцию), и раньше это означало печать ЕГО
  ПОЛНОГО тела N раз подряд — не косметика, а реальный риск на настоящем файле (не калибровочной
  фикстуре): 5 хитов, эскалировавших в 6571-строчную функцию, дали бы 30000+ строк вывода. Теперь
  тело печатается РОВНО ОДИН раз на уникальный `(start,end)`, порядок — как хиты шли во входном
  `--line` (не пересортировано по позиции, в отличие от survey/outline — REQ-001 §3 кейс B: порядок
  вывода = порядок запроса, это осознанный выбор автора запроса). Нумерация — `[i/n]`, но `n` = число
  УНИКАЛЬНЫХ блоков после дедупа, не число хитов — так дедуп никогда не путает счётчик. Никаких
  hit-строк и hit-счётчиков в заголовке (query не про «докажи что попал», это уже про «дай тело») и
  никакого текстового label рядом с `Block level: K range: A-B` — он бы всего лишь повторил первую
  строку тела одной строкой ниже, лишний риск разойтись. Плохой хит не роняет батч — `#ERROR: …`
  (без «hit», просто номер строки) и идёт дальше; ненулевой exit-код, если хоть один был.
