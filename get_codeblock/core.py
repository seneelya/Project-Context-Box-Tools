...
def parse_args():
    ...
    while i < len(tokens):
        ...
        if token == '--file' and i + 1 < len(tokens):
            ...
            print("  --outline --line N  FOCUS mode (single line, not the batch above): the table of")
            print("                      contents of just ONE named block — walk up --ancestor-level")
            print("                      NAMED ancestors from line N (anonymous try/if/for wrappers don't")
            print("                      count as a step); --level here picks WHICH block, not a depth")
            print("                      cap. Real use: you grepped/landed on a line deep inside some")
            print("                      control flow and don't know what function/class it's even in —")
