...
def parse_args():
    ...
    while i < len(tokens):
        ...
        if token == '--file' and i + 1 < len(tokens):
            ...
            print("  get_codeblock.py --file PATH --outline --line N [--level K]   (focus: one block's own map)")
            ...
            print("  --outline --line N --ancestor-level K")
            ...
            print("                      step); --level K works the same way (picks WHICH block, not a")
            print("                      depth cap here). Real use: you grepped/landed on a line deep")
            print("                      inside some control flow and don't know what function/class")
            print("                      it's even in — `--outline --line 49 --ancestor-level 1` shows")
            print("                      the outline of that containing named function.")
            print("                      Works with an array of hits too, same three flags:")
            print("                      `--outline --line 44,62 --level -2` escalates EACH hit the same")
