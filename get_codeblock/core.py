...
def parse_args():
    ...
    while i < len(tokens):
        ...
        if token == '--file' and i + 1 < len(tokens):
            ...
            print("")
            print("Flag order convention (flags are position-INDEPENDENT — the parser doesn't care,")
            print("this is just how commands are written/read): --project-root -> --file -> --line ->")
            print("--level/--ancestor-level -> --outline/--query -> --numbered. Address narrows first")
            print("(root -> file -> line -> block), then the verb acts, then adverbs.")
            print("")
            print("Arguments:")
            ...
            print("  --line N --level K --outline")
            print("                      FOCUS: the table of contents of the named block K ancestors up")
            print("                      from line N (anonymous try/if/for wrappers don't count as a")
            print("                      step); --ancestor-level K works the same way (picks WHICH block,")
            print("                      not a depth cap here). Real use: you grepped/landed on a line")
            print("                      deep inside some control flow and don't know what function/class")
            print("                      it's even in — `--line 49 --ancestor-level 1 --outline` shows")
            print("                      the outline of that containing named function.")
            print("                      Works with an array of hits too, same three flags:")
            print("                      `--line 44,62 --level -2 --outline` escalates EACH hit the same")
            print("                      way and merges shared ancestors into one tree, not N separate maps.")
