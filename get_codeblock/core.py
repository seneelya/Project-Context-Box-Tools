...
def parse_args():
    ...
    while i < len(tokens):
        ...
        if token == '--file' and i + 1 < len(tokens):
            ...
            print("  --outline --line N  FOCUS: the table of contents of the named block --ancestor-")
            print("                      level N ancestors up from line N (anonymous try/if/for wrappers")
            print("                      don't count as a step); --level here picks WHICH block, not a")
            print("                      depth cap. Works with an array too (--line a,b,...) — each hit")
            print("                      escalates the same way, shared ancestors merge into one tree.")
            print("                      Real use: you grepped/landed on a line deep inside some control")
            print("                      flow and don't know what function/class it's even in —")
            print("                      `--outline --line N --ancestor-level 1` shows the outline of")
            print("                      that containing named function.")
