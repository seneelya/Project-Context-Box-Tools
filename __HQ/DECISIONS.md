...
- Boundary: source-analysis tools (`codebase_import_search`, `get_codeblock`, `py_api`) are **fact-fetchers**; "who-calls-whom" / project map is the **card layer** (`rebuild_graph` over cards). Do NOT grow the fetchers into a graph system — anti-monster, fight for every flag.
