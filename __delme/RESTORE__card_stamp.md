...
> Точка восстановления для текущего потока: **штемпель карточки `card_api.py`** —
...
3. **`card_api.py`** — сам штемпель; докстринг вверху объясняет замысел и три факта.
...
`py card_api.py <file> --project-root R` → готовый `.md`-скелет карточки: ФАКТЫ заполнены
...
- **Не гонять golden после каждой правки** (он покрывает get_codeblock+import_search, НЕ card_api/format/py_api).
