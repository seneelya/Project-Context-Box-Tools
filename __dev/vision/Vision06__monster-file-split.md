...
сгенерированный (или, на простом v0, написанный мной вручную) скрипт: `cut(...) -> Block`,
`replace(..., replacement) -> Replace` (тот же резолв + текст на месте диапазона в source),
`add_import(text) -> импорт`, `monster.write(target_file, blocks, imports)` (запись/дозапись цели),
`monster.cut(source_file, blocks)` / `monster.replace(source_file, Replace[])` (освободить диапазон
в source: удалить или подставить stub). Вся механика (резолв через `get_codeblock`, порядок снизу
вверх, prepend-восстановление порядка) живёт ОДИН раз внутри этих примитивов — сам скрипт остаётся
тонким. **2026-09-23:** код в generate — `cut`+`monster.cut`; `.md` — `STUB_XX`+`replace`+
`monster.replace`; в каждом `move.py` — одна шапка с полной палитрой API (см. `split_monster__TLDR.md`).
...
```
(`--split LINE TARGET`, повторяется на каждый move — `nargs=2, action='append'`; в одном `LINE`
можно несколько номеров через запятую для одного target). Номера строк — те же, что я и
...
1. **`outline_rows` может резать диапазон МЕЛЬЧЕ, чем реально резолвит `cut()`** — комментарий
   ...
   `test_split_monster.py`; на 2026-09-23 — 27/27, incl. md/replace/comma-split/API-header).
...
**Частично закрыто в split_monster (2026-09-23):** `replace()` + `monster.replace()` — после
`monster.write` подставить stub в source на месте вырезанной секции (md-ссылка, deprecation в коде);
`replacement` — обычная строка с `\n`. Это не общий «replace строки N без переноса в другой файл».

**Отдельный будущий тул** (та же база — `get_codeblock`'s адресация по номеру строки — но in-place
без сценария split/write). Наблюдение владельца:
