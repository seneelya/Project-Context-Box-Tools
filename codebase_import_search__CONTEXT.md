...
- `--module-names NAME1,NAME2,...` — дополнительные имена по которым этот модуль можно импортировать (basename без `.py`, альтернативные имена). Часть имён резолвится автоматически из пути: basename (`auth.py` → `auth`), dotted path от project-root (`_core/auth.py` → `_core.auth`).
- `--language LNG` — язык (по умолчанию Python)
- `--project-root PATH` — корень проекта для поиска (опционально, дефолт = текущая директория)

Пример запуска:
```bash
codebase_import_search --file "_core/secret.py" --module-names "_secret_module,auth_core" --language python
# Автоматически добавит: secret (basename), _core.secret (dotted path)
```
...
- Ищем все вхождения `ALIAS.something` (или `target.something`) — **не только вызовы со скобками**
- Фиксируем всё что между алиасом и границей токена: точка + последовательность идентификаторов через точки (`foo.bar.baz`, `CONSTANT`, `handler`)
- Граница токена: пробел, конец строки, оператор (`+`, `=`, `,`, `)`, `;`, `[`, `:`), новая строка
- Пример: `fa.foo.function()` → фиксируем `foo.function` (всё между `fa.` и первой скобкой вызова)
- Пример: `fa.db_connect()` → фиксируем `db_connect`
- Пример: `x = fa.CONSTANT` → фиксируем `CONSTANT` (нет скобок, но есть доступ к атрибуту)
- Пример: `y = some_func(fa.handler)` → фиксируем `handler` (передан как аргумент без вызова)
...
## Важное уточнение: возвращаем путь к символу внутри целевого файла, а не алиас импорта

Алиас — это локальный артефакт файла-потребителя. Нам не интересен `fa`, нам интересны **символы внутри исследуемого файла**.

Правило простое: **всё между алиасом и границей токена — это путь к символу в файле.** Мы импортировали ФАЙЛ (или модуль), а не абстрактную переменную — поэтому сохраняем всю цепочку доступа до границы использования.
...
### Пример 3: without скобок — атрибуты, константы, передача как аргумента

```python
# src/config.py
import _core.auth as fa
TOKEN = fa.CONSTANT              # → CONSTANT (нет вызова, но есть доступ)
handler = some_func(fa.handler)  # → handler (передан без вызова)
path = fa.obj.nested_attr        # → obj.nested_attr
```

Вывод: `src/config.py: [CONSTANT, handler, obj.nested_attr]`

### Пример 4: from-import (без алиаса)
...
**Итоговое правило:** алиас импорта целевого файла — это «ключ». Снимаем его, возвращаем ВСЁ что между ним и границей токена (путь к символу внутри исследуемого файла). Скобки не обязательны — ловим любой доступ по атрибуту.
